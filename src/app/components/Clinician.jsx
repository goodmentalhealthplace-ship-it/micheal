...
export default function MichaelNwannaProfile() {
  ...
  return (
    <motion.section 
      ...
      className="w-full bg-[#F4F9FA] py-24" // Very light blue/gray background for emphasis
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-16">
        ...
        <div className="flex flex-col lg:flex-row bg-white rounded-[40px] shadow-2xl overflow-hidden border border-gray-100">
          ...
          <motion.div 
            ...
            className="w-full lg:w-1/3 p-8 bg-[#1B3C6A] text-white flex flex-col items-center justify-center" // Primary Navy background
          >
            <div className="relative w-48 h-48 mb-6">
              {/* IMPORTANT: REPLACE this placeholder with Michael Nwanna's actual professional headshot */}
              <Image
                src="/clinician-michael-nwanna.jpg" 
                alt="Profile photo of Michael Nwanna"
                fill
                sizes="(max-width: 1024px) 100vw, 33vw"
                className="rounded-full object-cover border-4 border-[#306EFF] shadow-lg" // Highlight Blue border
              />
            </div>
            ...
            <p className="text-lg font-semibold text-[#FFAA00] mb-6"> {/* COLOR: Accent Orange #FFAA00 */}
              APRN, CNP, PMHNP-BC
            </p>
            ...
            <div className="space-y-3 text-left w-full max-w-xs">
              {expertisePoints.map((point, i) => (
                <div key={i} className="flex items-start gap-3">
                  <point.icon className="text-[#306EFF] text-xl mt-1 flex-shrink-0" /> {/* COLOR: Highlight Blue #306EFF */}
          ...
          <div className="w-full lg:w-2/3 p-8 md:p-12 space-y-6">
            ...
            <motion.div variants={itemVariants} className="mt-8 text-center lg:text-left">
                <Link 
                  href="/contact"
                  className="inline-flex items-center gap-2 text-lg font-bold text-white px-8 py-4 rounded-2xl bg-[#306EFF] hover:bg-opacity-90 transition shadow-lg" // COLOR: Highlight Blue #306EFF
                >
